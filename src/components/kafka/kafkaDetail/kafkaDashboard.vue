      ...
      <el-row style="margin-top: 10px" type="flex" justify="space-around">
        <el-col :span="6">
          <div class="box-item bg-orange bg-height white-color">
            <h4 class="font-size-40 box-h margin-top20">{{data.clusterStatus}}</h4>
    ...
    methods: {
      getCount: function () {
        ...
        api.kafkaPackFunction(`/kafka/health`, "get", selectCluster).then((res) => {
          if (res.data.reply.result.data.status === 'ok') {
            this.data.clusterStatus = '正常'
          } else {
            this.data.clusterStatus = '异常'
          }
        })
